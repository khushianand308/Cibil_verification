...
def main():
    if not os.path.exists(LORA_PATH):
        print(f"ERROR: LoRA path tracker {LORA_PATH} not found. Is training finished?")
        return
    ...
    for entry in tqdm(test_data):
        ...
        pred = extract_json(prediction_str)
        valid_json = True
        if pred is None:
            valid_json = False
            pred = {
        ...
        flat_results.append({
            "gt_disp": gt.get("DISPOSITION", ""),
            "gt_name": bool(gt.get("NAME_VERIFIED", False)),
            "gt_loan": bool(gt.get("LOAN_NUMBER_VERIFIED", False)),
            "gt_rpc": gt.get("RPC_STATUS", ""),
            "pred_disp": pred.get("DISPOSITION", ""),
            "pred_name": bool(pred.get("NAME_VERIFIED", False)),
            "pred_loan": bool(pred.get("LOAN_NUMBER_VERIFIED", False)),
            "pred_rpc": pred.get("RPC_STATUS", ""),
    ...
    report_path = os.path.join(report_dir, "v2_evaluation_results.csv")
